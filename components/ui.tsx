...
export function Card({
  ...
  return (
    <section
      {...props}
      className={clsx("glass rounded-[24px] p-5", className)}
    >
      {children}
    </section>
  );
...
export function Button({
  ...
  return (
    <button
      {...props}
      className={clsx(
        "rounded-2xl px-4 py-2.5 font-semibold transition hover:-translate-y-0.5 active:translate-y-0",
        style,
        className
      )}
    >
...
export function StatCard({
  ...
  return (
    <Card className="flex min-h-[126px] items-center gap-4">
      <div
        className={clsx(
          "grid h-16 w-16 place-items-center rounded-full bg-gradient-to-br text-3xl text-white shadow-glow",
          color
        )}
      >
        {icon}
